...
- Store links: `VITE_APP_STORE_URL` and `VITE_GOOGLE_PLAY_URL` are empty, so store badges fall back to `#download`.
- Cover artwork: all hero, feature, blog, and avatar art uses fictional gradient covers from the handoff.
- Fictional titles: episode names such as "Nachtfunk", "Gruft 13", and "Der letzte Zug" are original placeholders.
- Product stats: `12.400`, `340k`, `½★`, and `100%` need real analytics or product approval.
...
- Font hosting: fonts load from Google Fonts; switch to local hosting if required by privacy/legal review.
